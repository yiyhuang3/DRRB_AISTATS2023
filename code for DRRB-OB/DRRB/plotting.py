...
# from DRRB.evaluation import eval_ate
from DRRB.loader import *
...
EARLY_STOP_CRITERION_BIN = 'roc_auc'
...
def select_parameters(output_dir, results, configs, stop_set, stop_criterion, choice_set, choice_criterion):
    ...
    np.savez(output_dir + '/' +'i_sel.npz', i_sel='i_sel')
...
def plot_evaluation_bin(results, configs, output_dir, data_train_path, data_test_path, filters=None, epsilon=0):
    early_stop_criterion = EARLY_STOP_CRITERION_BIN
    data_train = load_data(data_train_path)
    data_test = load_data(data_test_path)
